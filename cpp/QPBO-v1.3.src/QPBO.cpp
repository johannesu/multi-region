...
template <typename REAL> 
	QPBO<REAL>::QPBO(size_t node_num_max, size_t edge_num_max, void (*err_function)(char *))
	: node_num(0),
	  nodeptr_block(NULL),
	  changed_list(NULL),
	  ...
	  zero_energy(0)
{
	...
	nodes[0] = (Node*) mxMalloc(2*node_num_max*sizeof(Node));
	arcs[0] = (Arc*) mxMalloc(4*edge_num_max*sizeof(Arc));
	  ...
	  zero_energy(q.zero_energy)
{
	...
	nodes[0] = (Node*) mxMalloc(2*node_num_max*sizeof(Node));
	arcs[0] = (Arc*) mxMalloc(2*arc_num_max*sizeof(Arc));
	if (!nodes[0] || !arcs[0]) { if (error_function) (*error_function)("Not enough memory!"); exit(1); }
	...
	void QPBO<REAL>::reallocate_nodes(int node_num_max_new)
{
	...
	nodes[0] = (Node*) mxRealloc(nodes_old[0], 2*node_num_max*sizeof(Node));
...
{
	size_t arc_num_max_old = (int)(arc_max[0] - arcs[0]);
	size_t arc_num_max = arc_num_max_new; if (arc_num_max & 1) arc_num_max ++;
	code_assert(arc_num_max > arc_num_max_old);
	Arc* arcs_old[2] = { arcs[0], arcs[1] };

	arcs[0] = (Arc*) mxRealloc(arcs_old[0], 2*arc_num_max*sizeof(Arc));
